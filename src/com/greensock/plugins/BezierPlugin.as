...
public class BezierPlugin extends TweenPlugin {
    ...
    public static function _parseBezierData(values:Array, type:String, prepend:Object = null):Object {
        ...
        var obj:Object = {},
                ...
                a:Number, b:Number, c:Number, d:Number, cur:Array, props:Array, i:int, j:int, l:int, p:String, cnt:int,
                tmp:Object;
        if (soft && prepend) {
    ...
    protected static function _calculateControlPoints(a:Array, curviness:Number = 1, quad:Boolean = false, basic:Boolean = false, correlate:Boolean = false):void {
        var l:int = a.length - 1,
                ii:int = 0,
                cp1:Number = a[0].a,
                i:int, p1:Number, p2:Number, p3:Number, seg:Segment, m1:Number, m2:Number, mm:Number, cp2:Number,
                qb:Array, r1:Number, r2:Number, tl:Number;
        for (i = 0; i < l; i++) {
            seg = a[ii];
            p1 = seg.a;
            p2 = seg.d;
    ...
    private static function _addCubicLengths(a:Array, steps:Array, precision:uint = 6):void {
        var inc:Number = 1 / precision,
                j:int = a.length,
                d:Number, d1:Number, s:Number, da:Number, ca:Number, ba:Number, p:Number, i:int, inv:Number,
                bez:Segment, index:int;
        while (--j > -1) {
            bez = a[j];
            s = bez.a;
            da = bez.d - s;
    ...
    override public function setRatio(v:Number):void {
        var segments:int = this._segCount,
                ...
                curIndex:int, inv:Number, i:int, p:String, b:Segment, t:Number, val:Number, l:int, lengths:Array,
                curSeg:Array;
        if (this._timeRes == 0) {
