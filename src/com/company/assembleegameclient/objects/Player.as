...
public class Player extends Character {
    ...
    override protected function getTexture(param1:Camera, param2:int) : BitmapData
    {
        ...
        if(this.isShooting || param2 < attackStart_ + this.attackPeriod_)
        {
            ...
            _loc12_ = (param2 - attackStart_) % this.attackPeriod_ / this.attackPeriod_;
            _loc13_ = 2;
        }
        else if(moveVec_.x != 0 || moveVec_.y != 0)
        {
            _loc6_ = 3.5 / this.getMoveSpeed();
            if(moveVec_.y != 0 || moveVec_.x != 0)
            {
                facing_ = Math.atan2(moveVec_.y,moveVec_.x);
            }
            _loc12_ = param2 % _loc6_ / _loc6_;
            _loc13_ = 1;
        }
        if(Parameters.data.transmog != 0 && objectId_ == map_.player_.objectId_)
        {
            setTexture(Parameters.data.transmog);
        }
        else if(this.isHexed())
        {
        ...
        {
            _loc3_ = animatedChar_.imageFromFacing(facing_,param1,_loc13_,_loc12_);
        }
        var _loc9_:int = tex1Id_;
        var _loc4_:int = tex2Id_;
        if(fakeTex1 != -1)
        {
            _loc9_ = fakeTex1;
        }
        ...
        {
            _loc8_ = texturingCache_[this.nearestMerchant_];
            if(_loc8_ == null)
            {
            ...
            _loc9_ = this.nearestMerchant_.getTex1Id(tex1Id_);
            _loc4_ = this.nearestMerchant_.getTex2Id(tex2Id_);
        }
        else
        {
            _loc11_ = texturingCache_[_loc3_];
        }
        if(_loc11_ == null)
        {
            _loc11_ = TextureRedrawer.resize(_loc3_.image_,_loc3_.mask_,size_,false,_loc9_,_loc4_);
            if(this.nearestMerchant_ != null)
            {
                texturingCache_[this.nearestMerchant_][_loc3_] = _loc11_;
            }
            else
            {
                texturingCache_[_loc3_] = _loc11_;
            }
        }
        if(hp_ < maxHP_ * 0.2)
        {
            ...
            _loc11_ = CachingColorTransformer.transformBitmapData(_loc11_,_loc7_);
        }
        var _loc10_:BitmapData = texturingCache_[_loc11_];
        if(_loc10_ == null)
        {
            if(this == this.map_.player_)
            {
                if(Parameters.VHS == 1)
                {
                    _loc10_ = GlowRedrawer.outlineGlow(_loc11_,65280);
                }
                else if(Parameters.VHS == 2)
                {
                    _loc10_ = GlowRedrawer.outlineGlow(_loc11_,16768256);
                }
                else if(this.hasSupporterFeature(1))
                {
                    _loc10_ = GlowRedrawer.outlineGlow(_loc11_,13395711,1.4,false,true);
                }
                else
                {
                    _loc10_ = GlowRedrawer.outlineGlow(_loc11_,this.legendaryRank_ == -1?0:16711680);
                }
            }
            else if(this.hasSupporterFeature(1))
            {
            ...
            texturingCache_[_loc11_] = _loc10_;
        }
        ...
        {
            _loc10_ = CachingColorTransformer.alphaBitmapData(_loc10_,Parameters.data.alphaMan);
        }
        else if(this.isStasis || this.isPetrified)
        {
            _loc10_ = CachingColorTransformer.filterBitmapData(_loc10_,PAUSED_FILTER);
        }
        else if(this.isInvisible)
        {
            _loc10_ = CachingColorTransformer.alphaBitmapData(_loc10_,0.4);
        }
        return _loc10_;
    }
    ...
    {
        if(this.name_.toLowerCase() == Parameters.data.multiLeader.toLowerCase() && this.socketCreated && this.socket.connected)
        {
            this.send(param1,param2,1);
        }
    ...
    {
        var _loc2_:Array = this.socket.readUTFBytes(this.socket.bytesAvailable).split(":");
        this.setTarget(_loc2_[0],_loc2_[1],_loc2_[2]);
    }

    public function send(param1:Number, param2:Number, param3:Number) : void
    {
        this.socket.writeFloat(param1);
        this.socket.writeFloat(param2);
        this.socket.writeFloat(param3);
        this.socket.flush();
    }

    public function distance() : Number
    {
        var _loc1_:Number = this.tx - x_;
        var _loc2_:Number = this.ty - y_;
        return Math.sqrt(_loc1_ * _loc1_ + _loc2_ * _loc2_);
    }

    public function setTarget(param1:Number, param2:Number, param3:Number) : void
    {
        if(param3 == 0)
        {
            this.target = false;
        ...
        this.tx = param1;
        this.ty = param2;
        this.ta = Math.atan2(param2 - this.y_,param1 - this.x_);
        if(param3 == 1)
        {
            this.target = true;
        }
    ...
    public function close() : void
    {
        ...
        {
            if(this.name_.toLowerCase() == Parameters.data.multiLeader.toLowerCase())
            {
                this.send(0,0,0);
                Parameters.data.multiLeader = "";
            }
            this.socket.close();
        }
    }
    ...
    public function createSocket() : void
    {
        ...
        this.socket.addEventListener("connect",this.connectHandler);
        this.socket.addEventListener("error",this.onError);
        this.socket.addEventListener("ioError",this.ioError);
        this.socket.addEventListener("socketData",this.socketDataHandler);
    ...
    public function resolveSummonLifetime(objType:int):int {
        switch (objType) {
            ...
            case 0x89b4:
                return 12000; // Celestial Forest
            case 0x3801:
                return 10000; // Crystal
            case 0x89b6:
                return 10000; // Depths
            case 0x89b7:
                return 10000; // Hivemind
            case 0x3797:
            case 0x37a2:
            case 0x37a3:
            case 0x37a4:
                return 7000;  // B.O.O.M.
            case 0x3806:
                return 12600;    // North Pole
            case 0x4251:
            case 0x37a9:
                return 10200;    // Mechanical
            case 0x89b8:
                return 9200;  // Incubation
        }
    }
    ...
    public function shootAutoAimWeaponAngle(param1:int, param2:int) : Boolean
    {
        ...
        var _loc10_:ObjectProperties = ObjectLibrary.getPropsFromType(param1);
        this.attackPeriod_ = 1 / this.attackFrequency() * (1 / _loc10_.rateOfFire_);
        if(param2 < attackStart_ + this.attackPeriod_)
        {
            return false;
        }
        var _loc6_:Vector3D = new Vector3D(this.x_,this.y_);
        var _loc3_:Point = this.sToW(this.mousePos_.x,this.mousePos_.y);
        var _loc9_:Vector3D = new Vector3D(_loc3_.x,_loc3_.y);
        var _loc7_:ProjectileProperties = _loc10_.projectiles_[0];
        _loc4_ = this.calcAimAngle(_loc7_.calcAvgSpeed(Parameters.data.speedMul == 1?this.projectileSpeedMult:Parameters.data.speedMul,this.projectileLifeMult),_loc7_.calcMaxRange(Parameters.data.speedMul == 1?this.projectileSpeedMult:Parameters.data.speedMul,this.projectileLifeMult) + Parameters.data.aaDistance,_loc6_,_loc9_);
        if(_loc4_)
        {
            _loc8_ = Math.atan2(_loc4_.y - this.y_,_loc4_.x - this.x_);
            this.attackStart_ = param2;
            this.attackAngle_ = _loc8_ + (!!isUnstable?Math.random() * (3.14159265358979 / 6) - 3.14159265358979 / 12:0);
            this.doShoot(this.attackStart_,param1,ObjectLibrary.xmlLibrary_[param1],_loc8_,true,true,true);
            return true;
        }
    ...
    public function shootAutoAimAbilityAngle(param1:int, param2:int) : void
    {
        ...
        var _loc3_:int = 0;
        var _loc9_:* = null;
        var _loc6_:ProjectileProperties = null;
        var _loc8_:XML = ObjectLibrary.xmlLibrary_[param1];
        if(!this.canUseAltWeapon(param2,_loc8_))
        {
            return;
        }
        ...
        var _loc11_:Point = this.sToW(this.mousePos_.x,this.mousePos_.y);
        param2 = TimeUtil.getModdedTime();
        var _loc24_:* = this.objectType_;
        switch(_loc24_)
        {
            case 817:
                if(creeps.length >= 3)
                ...
                this.useAltWeapon(x_,y_,1,param2,true);
                return;
            case 784:
                ...
                return;
            case 768:
                _loc17_ = _loc8_.Activate;
                _loc21_ = 0;
                _loc15_ = new XMLList("");
                _loc19_ = _loc8_.Activate;
                _loc20_ = 0;
                _loc12_ = new XMLList("");
                var _loc22_:* = _loc8_.Activate;
                var _loc23_:int = 0;
                var _loc25_:* = new XMLList("");
                if(_loc8_.Activate.(text() == "Teleport") == "Teleport")
                {
                    return;
                }
            case 797:
            case 799:
                this.useAltWeapon(this.x_,this.y_,1,param2,true,_loc8_);
                this.lastAutoAbilityAttempt = param2;
                return;
            case 806:
            ...
            case 802:
                ...
                return;
            case 804:
                _loc13_ = _loc8_.Activate;
                _loc5_ = 0;
                _loc17_ = new XMLList("");
                _loc4_ = _loc8_.Activate;
                _loc16_ = 0;
                _loc19_ = new XMLList("");
                var _loc26_:* = _loc8_.Activate;
                var _loc27_:int = 0;
                _loc22_ = new XMLList("");
                if(_loc8_.Activate.(text() == "Teleport") == "Teleport")
                {
                ...
                _loc7_ = Parameters.data.spamPrismNumber;
                if(_loc7_ > 0)
                {
                    _loc3_ = 0;
                    _loc14_ = 0;
                    _loc18_ = this.map_.goDict_;
                    var _loc29_:int = 0;
                    var _loc28_:* = this.map_.goDict_;
                    for each(var _loc10_ in this.map_.goDict_)
                    {
                        if(_loc10_.props_.isEnemy_ && this.getDistSquared(this.x_,this.y_,_loc10_.x_,_loc10_.y_) <= 225)
                        {
                            _loc3_++;
                            if(_loc3_ > _loc7_)
                            {
            ...
            case 798:
            case 775:
                _loc6_ = ObjectLibrary.getPropsFromType(param1).projectiles_[0];
                if(_loc6_)
                {
                    _loc6_ = ObjectLibrary.getPropsFromType(param1).projectiles_[0];
                    _loc9_ = this.calcAimAngle(_loc6_.speed,_loc6_.maxProjTravel_,new Vector3D(this.x_,this.y_),new Vector3D(_loc11_.x,_loc11_.y),true);
                    if(_loc9_)
                    {
                        this.useAltWeapon(_loc9_.x,_loc9_.y,1,param2,true,_loc8_);
                        lastAutoAbilityAttempt = param2;
                    }
                }
                return;
            case 805:
                _loc9_ = this.calcAimAngle(NaN,7,new Vector3D(this.x_,this.y_),new Vector3D(_loc11_.x,_loc11_.y));
                if(_loc9_)
                {
                    this.useAltWeapon(_loc9_.x,_loc9_.y,1,param2,true,_loc8_);
                    lastAutoAbilityAttempt = param2;
                ...
                return;
            case 782:
                _loc9_ = this.calcAimAngle(NaN,12,new Vector3D(this.x_,this.y_),new Vector3D(_loc11_.x,_loc11_.y));
                if(_loc9_)
                {
                    this.useAltWeapon(_loc9_.x,_loc9_.y,1,param2,true,_loc8_);
                    lastAutoAbilityAttempt = param2;
                ...
                return;
            case 803:
                if(Parameters.data.mysticAAShootGroup)
                {
                    if(this.necroHeal())
                    {
                        this.lastAutoAbilityAttempt = param2;
                ...
                return;
            case 785:
                _loc9_ = this.calcAimAngle(NaN,getWakiRange(param1),new Vector3D(this.x_,this.y_),new Vector3D(_loc11_.x,_loc11_.y));
                if(_loc9_)
                {
                    this.useAltWeapon(_loc9_.x,_loc9_.y,1,param2,true,_loc8_);
                    lastAutoAbilityAttempt = param2;
    ...
    private function getMoveSpeed() : Number
    {
        ...
        if((this.isSpeedy || this.isNinjaSpeedy) && !Parameters.data.antiSpeedy)
        {
            _loc1_ = _loc1_ * 1.5;
        }
