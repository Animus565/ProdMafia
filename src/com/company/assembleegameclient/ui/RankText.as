...
public class RankText extends Sprite
{
    ...
    public function draw(param1:int, param2:int) : void
    {
        ...
        text.setStringBuilder(new StaticStringBuilder(!!Parameters.data.dataScrambler?(int(Math.random() * 85)).toString():this.numStars_.toString()));
        text.filters = [new DropShadowFilter(0,0,0,1,4,4,2)];
        this.background.addChild(text);
        this.icon = !this.largeText_?FameUtil.numStarsToImage(!!Parameters.data.dataScrambler?int(Math.random() * 85):int(this.numStars_)):FameUtil.numStarsToBigImage(!!Parameters.data.dataScrambler?int(Math.random() * 85):int(this.numStars_));
