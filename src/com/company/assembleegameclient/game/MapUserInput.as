...
public class MapUserInput {
    ...
    private function onEnterFrame(param1:Event):void {
        ...
        var _loc2_:* = null;
        var _loc6_:Number = NaN;
        var _loc7_:Player = this.gs.map.player_;
        var _loc5_:int = 0;
        if(_loc7_)
        {
            _loc7_.mousePos_.x = this.gs.map.mouseX;
            _loc7_.mousePos_.y = this.gs.map.mouseY;
            if(this.summCtrl)
            {
                _loc5_ = 0;
                while(_loc5_ < _loc7_.creeps.length)
                {
                    if(_loc7_.creepMovable[_loc5_])
                    {
                        _loc3_ = _loc7_.sToW(this.gs.map.mouseX,this.gs.map.mouseY);
                        _loc2_ = this.gs.map.lookupSquare(_loc3_.x,_loc3_.y);
                        if(_loc2_ && _loc2_.isWalkable())
                        {
                            this.gs.gsc_.moveCreep(_loc3_.x,_loc3_.y,_loc7_.creeps[_loc5_],true);
                        }
                    }
                    _loc5_++;
                }
            ...
            {
                var _loc9_:* = Parameters.data.autoSummCtrlMode;
                switch(_loc9_)
                {
                    case 1:
                        _loc5_ = 0;
                        while(_loc5_ < _loc7_.creeps.length)
                        {
                            if(!(_loc7_ == null || !_loc7_.creepMovable[_loc5_]))
                            {
                                _loc4_ = this.gs.map.goDict_[_loc7_.creeps[_loc5_]];
                                if(!_loc4_)
                                {
                                    if(_loc7_.square.isWalkable() && _loc7_.creepMovable[_loc5_])
                                    {
                                        this.gs.gsc_.moveCreep(_loc7_.x_,_loc7_.y_,_loc7_.creeps[_loc5_],false);
                                    }
                                }
                                else
                                {
                                    _loc8_ = PointUtil.distanceSquaredXY(_loc4_.x_,_loc4_.y_,_loc7_.x_,_loc7_.y_) > 25;
                                    if(_loc7_.square && _loc7_.square.isWalkable())
                                    {
                                        this.gs.gsc_.moveCreep(_loc7_.x_,_loc7_.y_,_loc7_.creeps[_loc5_],!_loc8_);
                                    }
                                }
                            }
                            _loc5_++;
                    ...
                    case 2:
                        ...
                        while(_loc5_ < _loc7_.creeps.length)
                        {
                            ...
                            {
                                _loc4_ = this.gs.map.goDict_[_loc7_.creeps[_loc5_]];
                                if(!_loc4_ || !_loc7_.summonerTarget || !_loc7_.summonerTarget.square || PointUtil.distanceSquaredXY(_loc4_.x_,_loc4_.y_,_loc7_.x_,_loc7_.y_) > 196)
                                {
                                    if(_loc7_.square.isWalkable())
                                    {
                                        this.gs.gsc_.moveCreep(_loc7_.x_,_loc7_.y_,_loc7_.creeps[_loc5_],false);
                                    }
                                }
                                else if(!(PointUtil.distanceSquaredXY(_loc7_.x_,_loc7_.y_,_loc7_.summonerTarget.x_,_loc7_.summonerTarget.y_) > 196 || PointUtil.distanceSquaredXY(_loc4_.x_,_loc4_.y_,_loc7_.summonerTarget.x_,_loc7_.summonerTarget.y_) < 0.25))
                                {
                                    _loc8_ = PointUtil.distanceSquaredXY(_loc4_.x_,_loc4_.y_,_loc7_.summonerTarget.x_,_loc7_.summonerTarget.y_) > 25;
                                    if(_loc7_.summonerTarget.square.isWalkable())
                                    {
                                        this.gs.gsc_.moveCreep(_loc7_.summonerTarget.x_,_loc7_.summonerTarget.y_,_loc7_.creeps[_loc5_],!_loc8_);
                                    }
                                }
                            }
                            _loc5_++;
            ...
            if(this.enablePlayerInput_)
            {
                if(this.mouseDown_)
                {
                    _loc6_ = Math.atan2(this.gs.map.mouseY,this.gs.map.mouseX);
                    _loc7_.attemptAttackAngle(_loc6_);
                    _loc7_.attemptAutoAbility(_loc6_);
                }
                else if(Parameters.data.AAOn || this.autofire_ || Parameters.data.AutoAbilityOn)
                {
                    _loc7_.attemptAutoAim(Math.atan2(this.gs.map.mouseY,this.gs.map.mouseX) + (!!_loc7_.isUnstable?Math.random() * (3.14159265358979 / 6) - 3.14159265358979 / 12:0));
                }
            }
        }
    }
        ...
        switch (_loc26_) {
            case Parameters.data.noClipKey:
                if (Parameters.data.noClip) {
                    ...
                    this.gs.moveRecords_.addRecord(player.x_,player.y_,TimeUtil.getModdedTime());
                }
            ...
            case Parameters.data.summCtrl:
                this.summCtrl = true;
                _loc111_ = 0;
                while(_loc111_ < player.creeps.length)
                {
                    if(player.creepMovable[_loc19_])
                    {
                        _loc11_ = player.sToW(this.gs.map.mouseX,this.gs.map.mouseY);
                        this.gs.gsc_.moveCreep(_loc11_.x,_loc11_.y,_loc111_,false);
                    }
                    _loc111_++;
                }
                return;
            case Parameters.data.pickUpKey:
                var vaultObject: GameObject = null;
                for each (var go: GameObject in gs.map.goDict_)
                    if (go.objectType_ == 0x504) {
                        vaultObject = go;
                        break;
                    }
                var emptySlots: Vector.<int> = new Vector.<int>();
                var i: int;
                var j: int = 0;
                for(i = 4; i < player.equipment_.length; i++)
                ...
                return;
            case Parameters.data.reconKey:
                if (Parameters.lastRecon) {
                    trace("Sending reconnect");
                    gs.dispatchEvent(Parameters.lastRecon);
                } else {
                    this.gs.map.player_.textNotification("Can't recon! No valid reconnect destination", 16777215, 2000, false);
                }
                return;
