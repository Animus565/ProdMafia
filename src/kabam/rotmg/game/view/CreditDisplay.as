...
public class CreditDisplay extends Sprite
{
    ...
    public function CreditDisplay(param1:GameSprite = null, param2:Boolean = true, param3:Number = 0)
    {
        ...
        var _loc4_:BitmapData = BitmapUtil.cropToBitmapData(AssetLibrary.getImage("fireforge_points"),4,4,8,8);
        _loc4_ = TextureRedrawer.redraw(_loc4_,40,true,0);
        this.forgefireIcon = new Bitmap(_loc4_);
        ...
        _loc4_ = AssetLibrary.getImageFromSet("lofiObj3",225);
        _loc4_ = TextureRedrawer.redraw(_loc4_,40,true,0);
        this.coinIcon_ = new Bitmap(_loc4_);
    ...
    {
        if(param1 == this.credits_ && (this.displayFame_ && param2 == this.fame_) && param3 == this.forgefire)
        {
            return;
        }
        this.forgefire = param3;
        this.forgefireText.setStringBuilder(new StaticStringBuilder(!Parameters.data.dataScrambler?this.forgefire.toString():(int(Math.random() * 1600)).toString()));
        this.credits_ = param1;
        this.creditsText_.setStringBuilder(new StaticStringBuilder(!Parameters.data.dataScrambler?this.credits_.toString():(int(Math.random() * 100000)).toString()));
        if(this.displayFame_)
        {
            this.fame_ = param2;
            this.fameText_.setStringBuilder(new StaticStringBuilder(!Parameters.data.dataScrambler?this.fame_.toString():(int(Math.random() * 100000)).toString()));
        }
        if(waiter.isEmpty())
        {
