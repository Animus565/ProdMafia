    ...
    public function execute():void {
        ...
        switch (command) {
            ...
            case "/pu":
                if(split.length < 2)
                {
                    this.addTextLine.dispatch(ChatMessage.make("*Help*","Your Projectile Update Rate is: " + Parameters.data.projUpdateRate + " ms"));
                    return;
                }
                Parameters.data.projUpdateRate = parseInt(split[1]);
                Parameters.save();
                this.hudModel.gameSprite.refreshProjUpdateTimerStatus();
                this.addTextLine.dispatch(ChatMessage.make("*Help*","Projectile Update Rate set to: " + Parameters.data.projUpdateRate + " ms"));
                return;
                break;
            case "/setleader":
                if(split.length > 1)
                {
                    Parameters.data.multiLeader = split[1];
                }
                else
                {
                    this.hudModel.gameSprite.map.player_.send(0,0,0);
                    Parameters.data.multiLeader = "";
                    this.addTextLine.dispatch(ChatMessage.make("*Help*","Multibox leader unset."));
                }
                Parameters.save();
                this.addTextLine.dispatch(ChatMessage.make("*Help*","Multibox leader set to: " + Parameters.data.multiLeader));
                return;
                break;
            case "/transmog":
                if(split.length < 2)
                {
                    player.setTexture(player.skinId != 0?player.skinId:int(player.objectType_));
                    Parameters.data.transmog = 0;
                    Parameters.save();
                    this.addTextLine.dispatch(ChatMessage.make("*Help*","Transmog unset."));
                    return;
                }
                Parameters.data.transmog = parseInt(split[1]);
                player.setTexture(Parameters.data.transmog);
                Parameters.save();
                var xml:XML = ObjectLibrary.xmlLibrary_[Parameters.data.transmog];
                this.addTextLine.dispatch(ChatMessage.make("*Help*","You have transmogrified into: \'" + (xml.DisplayId != undefined?xml.DisplayId:xml.@id) + "\'"));
                return;
                break;
            default:
