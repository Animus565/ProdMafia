...
public class GameServerConnectionConcrete extends GameServerConnection {
    ...
    private function updateGameObject(param1:GameObject, param2:Vector.<StatData>, param3:Boolean, param4:Boolean) : void
    {
        ...
        _loc8_ = 0;
        for(; _loc8_ < _loc10_; _loc8_ = Number(_loc8_) + 1)
        {
            ...
            switch(_loc6_)
            {
                ...
                case 19:
                    ...
                    if(param3 && _loc11_ > 3 && Parameters.data.autoStack)
                    {
                        _loc12_.checkStacks = true;
                        continue;
                    }
                    continue;
                case 30:
                ...
                case 114:
                    ...
                    {
                        setTimeout(function(param1:int):void
                        {
                            if(!gs_ || !gs_.map || !gs_.map.goDict_)
                            {
                                return;
                            }
                            var _loc2_:int = gs_.map.player_.creeps.indexOf(param1);
                            if(_loc2_ != -1)
                            {
                                gs_.map.player_.creeps.removeAt(_loc2_);
                                gs_.map.player_.creepMovable.removeAt(_loc2_);
                            }
                        },gs_.map.player_.resolveSummonLifetime(go.objectType_) - 200,go.objectId_);
                        if(movable)
                        {
                            _loc6_ = Parameters.data.autoSummCtrlMode;
                            switch(_loc6_)
                            {
                                case 0:
                ...
                case 120:
                    ...
                    continue;
                default:
                    trace("Unhandled stat type:",statType,"statVal:",statVal,"name:",go.name_);
                    continue;
            }
        }
        if(param3)
